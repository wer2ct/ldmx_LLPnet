...
#Use like --> python3 GraphMaker.py <Input .npz file> <outpath> <tag>
...
def main():
    ...
    for i in np.unique(hcal_hits_array[:,1]):
        ...
        #HCal Edges
        hcal_edges = set() #automatically remove any duplicates (there shouldn't be any but just in case I'm silly)
        for k in range(len(hcal_node_features)):
            for j in range(len(hcal_node_features)):
                if (k != j): #no self connected nodes. 
                    hcal_edges.add((k,j))
            
        #ECal Edges
        ecal_edges = set() #automatically remove any duplicates (there shouldn't be any but just in case I'm silly)
        for k in range(len(ecal_node_features)):
            for j in range(len(ecal_node_features)):
                if (k != j): #no self connected nodes. 
                    ecal_edges.add((k,j))

        hcal_edge_index = torch.tensor(list(hcal_edges), dtype=torch.long).t().contiguous()
        ecal_edge_index = torch.tensor(list(ecal_edges), dtype=torch.long).t().contiguous()
        ...
        #We now need to shift our HCal indices in the edge index by the number of ECal nodes. 
        hcal_edge_index_shifted = hcal_edge_index + N_ECal

        #now lets combine our edge indices, and create a vector to identify the detectors the hits are in
        edge_index_combined = torch.cat([ecal_edge_index, hcal_edge_index_shifted], dim=1)
        det_idx = torch.cat([
        ...
        event_data = Data(
            ...
            edge_index=edge_index_combined,   # Edge Indices [2, E_Total]
